...
class PermutasController extends BaseController
{
    ...
    public function index()
    {
        ...
        // Bloquear acesso de assistentes operacionais (nível 0)
        if ($userLevel == 0) {
    ...
    public function salvarPermuta()
    {
        ...
        log_message('info', '=== CRIAÇÃO DE PERMUTA - DEBUG ANO LETIVO ===');
        log_message('info', 'Ano Ativo: ' . json_encode($anoAtivo));
        log_message('info', 'Ano Letivo ID extraído: ' . ($anoLetivoId ?? 'NULL'));
        ...
        try {
            ...
            $blocoInicial = $post['bloco_reposicao'] ?? null;
            $indiceBlocoAtual = 0; // Índice para incrementar os blocos em grupos de permutas
            
            foreach ($aulasParaPermutar as $aulaId) {
                // Para grupos de permutas, atribuir blocos consecutivos
                $blocoReposicaoId = $blocoInicial;
                if ($grupoPermuta && $blocoInicial) {
                    $blocoReposicaoId = $blocoInicial + $indiceBlocoAtual;
                    // Garantir que o bloco não ultrapasse o máximo (12 blocos)
                    if ($blocoReposicaoId > 12) {
                        $blocoReposicaoId = 12;
                    }
                    $indiceBlocoAtual++;
                }
                ...
                $permutaData = [
                    ...
                    'bloco_reposicao_id'        => $blocoReposicaoId,
                    'grupo_permuta'             => $grupoPermuta,
                    'estado'                    => 'pendente',
                    'observacoes'               => $post['observacoes'] ?? null
                ];
                
                log_message('info', 'Dados ANTES do insert: ' . json_encode($permutaData));

                $permutaId = $this->permutaModel->insert($permutaData);
                
                log_message('info', 'Permuta inserida com ID: ' . $permutaId);
                
                // Verificar na DB se o ano_letivo_id foi realmente inserido
                $permutaInserida = $this->permutaModel->find($permutaId);
                log_message('info', 'Dados DEPOIS do insert (da DB): ' . json_encode($permutaInserida));
    ...
    public function aprovarPermuta($permutaId)
    {
        ...
        if ($success) {
            // Log da aprovação (após o JSON ser enviado)
            try {
                log_permuta(
                    'approve',
                    $permutaId,
                ...
                log_message('error', 'Erro no log da permuta: ' . $e->getMessage());
            }
    ...
    public function testeEmail()
    {
        ...
        try {
            ...
            if ($email->send()) {
                echo "<p style='color: green;'>✅ Email enviado para escoladigitaljb@aejoaodebarros.pt</p>";
            } else {
                echo "<p style='color: red;'>❌ Erro ao enviar para escoladigitaljb@aejoaodebarros.pt</p>";
            }
        } catch (\Exception $e) {
            echo "<p style='color: red;'>❌ Erro: " . $e->getMessage() . "</p>";
        }
    ...
    /**
     * Página de Permutas Aprovadas (para níveis 0, 8 e 9)
     */
    public function aprovadas()
    {
        ...
        // Obter lista de escolas para filtro
        $escolas = $this->db->table('escolas')->select('id, nome as nome_escola')->orderBy('nome', 'ASC')->get()->getResultArray();

        // Obter semanas do ano letivo atual
        $anoLetivoAtivo = $this->anoLetivoModel->where('status', 1)->first();
        $semanas = [];
        $semanaAtual = (int)date('W');
        ...
        $data = [
            'title' => 'Permutas Aprovadas',
            'page_title' => 'Visualização de Permutas Aprovadas',
            'page_subtitle' => 'Consultar permutas aprovadas com filtros avançados',
            'escolas' => $escolas,
            'semanas' => $semanas,
            'semanaAtual' => $semanaAtual
        ];

        return view('permutas/aprovadas', $data);
    }

    /**
     * Obter dados de permutas aprovadas via AJAX
     */
    public function getPermutasAprovadas()
    {
        $userData = session()->get('LoggedUserData');
        $userLevel = $userData['level'] ?? 0;
        ...
        // Obter filtros
        $filtroDataInicio = $this->request->getGet('data_inicio');
        $filtroDataFim = $this->request->getGet('data_fim');
        $filtroEscola = $this->request->getGet('escola');
        $filtroPavilhao = $this->request->getGet('pavilhao');

        // Query base
        $builder = $this->db->table('permutas p');
        $builder->select('p.*, 
                         ha.codigo_turma, ha.disciplina_id,
                         autor.name as professor_autor_nome,
                         substituto.name as professor_substituto_nome,
                         s.codigo_sala, s.descricao as sala_descricao,
                         bh.hora_inicio, bh.hora_fim, bh.designacao as bloco_designacao,
                         e.nome as nome_escola,
                         t.escola_id');
        $builder->join('horario_aulas ha', 'ha.id_aula = p.aula_original_id', 'left');
        $builder->join('turma t', 't.codigo = ha.codigo_turma', 'left');
        $builder->join('user autor', 'autor.NIF = p.professor_autor_nif', 'left');
        $builder->join('user substituto', 'substituto.NIF = p.professor_substituto_nif', 'left');
        ...
        if ($filtroDataFim) {
            $builder->where('p.data_aula_permutada <=', $filtroDataFim);
        }

        // Aplicar filtro de escola
        if ($filtroEscola) {
            $builder->where('t.escola_id', $filtroEscola);
        }

        // Aplicar filtro de pavilhão (baseado no código da sala)
        if ($filtroPavilhao) {
            $builder->like('p.sala_permutada_id', $filtroPavilhao, 'after');
        }

        $builder->orderBy('p.data_aula_permutada', 'DESC');
        $builder->orderBy('bh.hora_inicio', 'ASC');

        $permutas = $builder->get()->getResultArray();

        return $this->response->setJSON(['data' => $permutas]);
    }

    /**
     * Gerar PDF com permutas aprovadas (com filtros)
     */
    public function gerarPdfAprovadas()
    {
        $userData = session()->get('LoggedUserData');
        $userLevel = $userData['level'] ?? 0;
        ...
        // Query base (mesma lógica do getPermutasAprovadas)
        $builder = $this->db->table('permutas p');
        $builder->select('p.*, 
                         ha.codigo_turma, ha.disciplina_id,
                         autor.name as professor_autor_nome,
                         ...
                         e.nome as nome_escola,
                         t.escola_id,
                         d.abreviatura as disciplina_abrev');
        $builder->join('horario_aulas ha', 'ha.id_aula = p.aula_original_id', 'left');
        $builder->join('turma t', 't.codigo = ha.codigo_turma', 'left');
        ...
        $builder->orderBy('p.data_aula_permutada', 'ASC');
        $builder->orderBy('bh.hora_inicio', 'ASC');

        $permutas = $builder->get()->getResultArray();

        if (empty($permutas)) {
            return redirect()->back()->with('error', 'Não há permutas aprovadas com os filtros aplicados');
        }
        ...
        $data = [
            ...
            'filtroPavilhao' => $filtroPavilhao,
            'totalPermutas' => count($permutas)
        ];

        // Gerar HTML
        $html = view('permutas/pdf_permutas_aprovadas', $data);
        ...
        // Nome do arquivo
        $filename = 'Permutas_Aprovadas_' . date('Y-m-d_His') . '.pdf';
