...
<body>
<?php 
    // Determinar o prefixo correto para URLs públicas
    // Em localhost: /public/
    // Em produção com subdomínio público: raiz /
    $isPublicSubdomain = false;
    $publicHost = getenv('PUBLIC_HOST') ?: env('PUBLIC_HOST');
    $currentHost = $_SERVER['HTTP_HOST'] ?? '';
    
    if (!empty($publicHost) && strcasecmp($currentHost, $publicHost) === 0) {
        // Estamos no subdomínio público em produção
        $publicPrefix = '/';
        $isPublicSubdomain = true;
    } else {
        // Localhost ou domínio principal - usar /public/
        $publicPrefix = base_url('public/');
    }
?>
<nav class="navbar navbar-expand-lg navbar-light bg-white sticky-top">
    <div class="container">
        <a class="navbar-brand d-flex align-items-center" href="<?= $publicPrefix ?>">
        ...
        <div class="collapse navbar-collapse" id="navbarMain">
            <ul class="navbar-nav ms-auto align-items-center">
                <li class="nav-item"><a class="nav-link" href="<?= $publicPrefix ?>"><i class="bi bi-house-door"></i> Início</a></li>
                <li class="nav-item"><a class="nav-link" href="<?= $publicPrefix . ($isPublicSubdomain ? 'about' : 'about') ?>"><i class="bi bi-info-circle"></i> Sobre</a></li>
                
                <!-- Dropdown Kit Digital -->
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="#" id="kitDigitalDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                        <i class="bi bi-laptop"></i> Kit Digital
                    </a>
                    <ul class="dropdown-menu" aria-labelledby="kitDigitalDropdown">
                        <li><a class="dropdown-item" href="<?= $publicPrefix . ($isPublicSubdomain ? 'kit-digital' : 'kit-digital') ?>">
                            <i class="bi bi-file-earmark-plus"></i> Requisição
                        </a></li>
                        <li><a class="dropdown-item" href="<?= $publicPrefix . ($isPublicSubdomain ? 'reportar-avaria-kit' : 'reportar-avaria-kit') ?>">
                            <i class="bi bi-exclamation-triangle"></i> Reportar Avaria
                        </a></li>
                    </ul>
                </li>
