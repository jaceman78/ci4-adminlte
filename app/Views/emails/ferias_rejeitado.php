<!DOCTYPE html>
<html lang="pt">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Pedido de Férias Rejeitado</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            line-height: 1.6;
            color: #333;
            max-width: 600px;
            margin: 0 auto;
            padding: 20px;
        }
        .header {
            background-color: #dc3545;
            color: white;
            padding: 20px;
            text-align: center;
            border-radius: 5px 5px 0 0;
        }
        .content {
            background-color: #f8f9fa;
            padding: 20px;
            border: 1px solid #dee2e6;
        }
        .info-box {
            background-color: white;
            padding: 15px;
            margin: 15px 0;
            border-left: 4px solid #dc3545;
            border-radius: 4px;
        }
        .periodo {
            padding: 10px;
            margin: 8px 0;
            background-color: #f8f9fa;
            border-radius: 4px;
            display: flex;
            justify-content: space-between;
            align-items: center;
        }
        .periodo-datas {
            font-weight: bold;
            color: #212529;
        }
        .periodo-dias {
            background-color: #6c757d;
            color: white;
            padding: 4px 12px;
            border-radius: 20px;
            font-weight: bold;
        }
        .alert-danger {
            background-color: #f8d7da;
            border: 1px solid #dc3545;
            padding: 15px;
            margin: 15px 0;
            border-radius: 4px;
            color: #721c24;
        }
        .alert-info {
            background-color: #d1ecf1;
            border: 1px solid #17a2b8;
            padding: 15px;
            margin: 15px 0;
            border-radius: 4px;
            color: #0c5460;
        }
        .motivo-box {
            background-color: #fff;
            border: 2px solid #dc3545;
            padding: 15px;
            margin: 15px 0;
            border-radius: 4px;
        }
        .button {
            display: inline-block;
            padding: 12px 30px;
            background-color: #007bff;
            color: white;
            text-decoration: none;
            border-radius: 5px;
            margin: 20px 0;
            text-align: center;
        }
        .footer {
            text-align: center;
            padding: 20px;
            color: #6c757d;
            font-size: 0.9em;
        }
    </style>
</head>
<body>
    <div class="header">
        <h2>❌ Pedido de Férias Não Aprovado</h2>
    </div>
    
    <div class="content">
        <p>Exmo(a). Sr(a). <strong><?= esc($pedido['nome_professor']) ?></strong>,</p>
        
        <div class="alert-danger">
            <strong>Informação:</strong> O seu pedido de férias para o ano <strong><?= $pedido['ano'] + 1 ?></strong> foi <strong>REJEITADO</strong> pelos serviços administrativos.
        </div>
        
        <p>Os períodos solicitados eram:</p>
        
        <div class="info-box">
            <?php foreach ($pedido['periodos'] as $index => $periodo): ?>
                <div class="periodo">
                    <div class="periodo-datas">
                        <?= date('d/m/Y', strtotime($periodo['data_inicio'])) ?> 
                        a 
                        <?= date('d/m/Y', strtotime($periodo['data_fim'])) ?>
                    </div>
                    <div class="periodo-dias">
                        <?= $periodo['dias_uteis'] ?> dia<?= $periodo['dias_uteis'] != 1 ? 's' : '' ?>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
        
        <?php if (!empty($motivo)): ?>
        <div class="motivo-box">
            <strong style="color: #dc3545;">Motivo da Rejeição:</strong>
            <p style="margin: 10px 0 0 0;">
                <?= nl2br(esc($motivo)) ?>
            </p>
        </div>
        <?php endif; ?>
        
        <div class="alert-info">
            <strong>📋 O que pode fazer:</strong>
            <ul style="margin: 10px 0; padding-left: 20px;">
                <li>Contacte os serviços administrativos para esclarecimentos</li>
                <li>Pode submeter um novo pedido com datas diferentes</li>
                <li>Verifique o seu saldo de dias disponíveis no sistema</li>
            </ul>
        </div>
        
        <div style="text-align: center;">
            <a href="<?= base_url('ferias') ?>" class="button">Aceder ao Sistema de Férias</a>
        </div>
        
        <p style="margin-top: 20px;">
            <strong>Nota:</strong> Não responda a este email. Para qualquer esclarecimento, contacte os serviços administrativos.
        </p>
    </div>
    
    <div class="footer">
        <p>Esta é uma mensagem automática do Sistema de Gestão Escolar</p>
        <p>Agrupamento de Escolas João de Barros</p>
        <p style="font-size: 0.85em; color: #999; margin-top: 10px;">
            Pedido #<?= $pedido['id'] ?> | Rejeitado em <?= date('d/m/Y', strtotime($pedido['rejeitado_em'])) ?>
        </p>
    </div>
</body>
</html>
